    ...
    render() {
        return (
            <div>
                ...
                <VictoryPie
                    data={[
                            { y: this.props.orderdateDim.group(d => d >= 6).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value - this.props.orderdateDim.group(d => d >= 12).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value, label: "Morning" },
                            { y: this.props.orderdateDim.group(d => d >= 12).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value - this.props.orderdateDim.group(d => d >= 17).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value, label: "Afternoon" },
                            { y: this.props.orderdateDim.group(d => d >= 17).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value - this.props.orderdateDim.group(d => d >= 20).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value, label: "Evening" },
                            { y: this.props.orderdateDim.group(d => d >= 20).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[1].value + this.props.orderdateDim.group(d => d >= 6).reduceSum(d => parseFloat(d.orderAmount.replace(/[^0-9.-]+/g, ''))).all()[0].value, label: "Night" }
                        ]}
                    height={200}
                    colorScale="warm"
                    labelRadius={60}
